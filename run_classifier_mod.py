...
class DataProcessor(object):
  ...
  def get_examples_from_df(self, df):
    """Gets a collection of `InputExample`s from DataFrame."""
    raise NotImplementedError()
...
class ImdbProcessor(DataProcessor):
  ...
  def get_examples_from_df(self, df):
    return self._create_examples_from_df(df)
  ...
  def _create_examples_from_df(self, df):
    examples = []
...
def main(_):
  ...
  #################################################################################################
    #dhiraj
    #load training and dev data from the csv file 
    df_train = pd.read_csv(os.path.join(FLAGS.data_dir, "train.csv"))
    df_valid = pd.read_csv(os.path.join(FLAGS.data_dir, "valid.csv"))
    ...
    labels = df_train['condition'].unique()
  ...
  if FLAGS.do_train:
    ...
    train_examples = processor.get_examples_from_df(df_train)
  ...
  if FLAGS.do_eval or FLAGS.do_predict:
    if FLAGS.eval_split == "dev":
      ...
      eval_examples = processor.get_examples_from_df(df_valid)
    else:
